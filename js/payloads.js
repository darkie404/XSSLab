...
class PayloadsManager {
  ...
  bookmarkPayload(payload, button) {
    const bookmarks = JSON.parse(localStorage.getItem('XSSLab-bookmarks') || '[]');
    const isBookmarked = bookmarks.some(b => b.code === payload.code);

    if (isBookmarked) {
      const filteredBookmarks = bookmarks.filter(b => b.code !== payload.code);
      localStorage.setItem('XSSLab-bookmarks', JSON.stringify(filteredBookmarks));
      ...
      localStorage.setItem('XSSLab-bookmarks', JSON.stringify(bookmarks));
      button.innerHTML = '<i class="fas fa-bookmark"></i>';
      button.style.color = 'var(--neon-green)';
    }
  }
  ...
  downloadFiltered() {
    ...
    const filename = `XSSLab-filtered-payloads-${timestamp}.txt`;
